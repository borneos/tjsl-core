        ...
        <div class="app-page-title">
            <div class="page-title-wrapper">
                <div class="page-title-heading">
                    ...
                    <div>Master Product <span
                            class="badge badge-pill badge-primary">{{ number_format($products->total(), 0, '', '.') }}</span>
                        <div class="page-title-subheading">List Master Product</div>
                    </div>
                </div>
            ...
            <div class="card-body">
                            ...
                            <div class="d-flex">
                                <div class="input-group w-100">
                                    <select style="font-size: 14px" name="status" id="status" class="form-control">
                                        <option {{ $status == 404 ? 'selected' : '' }} disabled selected>Select Status
                                        </option>
                                        <option {{ $status == 1 ? 'selected' : '' }} value="1">Status Active</option>
                                        <option {{ $status == 0 ? 'selected' : '' }} value="0">Status Not Active
                                        </option>
                                    </select>
                                </div>
                            </div>
                ...
                <table style="width: 100%;" class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                <td>{{ $product->id }}</td>
                                <td><img src="{{ URL::to($product->compressImage('w_32,h_32')) }}" width="32"
                                        height="32" alt=""></td>
                                <td>{{ $product->sku ?? '-' }}</td>
                                ...
                                <td>
                                    <label class="m-auto align-middle" for="statusCheckbox{{ $product->id }}">
                                        <input type="checkbox" data-toggle="toggle" data-size="small"
                                            onChange="location.href='{{ route('admin.product.status', [$product['id'], $product->status ? 0 : 1]) }}'"
                                            id="statusCheckbox{{ $product->id }}"
                                ...
                                <td>
                                    <label class="m-auto align-middle" for="favoriteCheckbox{{ $product->id }}">
                                        <input type="checkbox" data-toggle="toggle" data-size="small"
                                            onChange="location.href='{{ route('admin.product.favorite', [$product['id'], $product->favorite ? 0 : 1]) }}'"
                                            id="favoriteCheckbox{{ $product->id }}"
                                            {{ $product->favorite ? 'checked' : '' }}>
                                    </label>
                                </td>
                                <td>
                                    <a href="{{ route('admin.product.edit', $product->id) }}"
                                        class="btn btn-warning btn-sm" title="Edit ?"><i style="font-size: 14px"
                                            class="text-white pe-7s-note"></i></a>
                                    <button type="button" onclick="delete_product({{ $product->id }})"
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 w-100 d-flex justify-content-end align-middle">
                        <p>Displaying {{ $products->count() }} of {{ number_format($products->total(), 0, '', '.') }}
                            product</p>
                    </div>
                </div>
            </div>
